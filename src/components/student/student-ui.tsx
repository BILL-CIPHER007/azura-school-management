    ...
    <div className="rounded-lg border border-border bg-surface p-3.5 shadow-sm">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <strong className={cn("mt-1 block truncate text-2xl font-semibold text-school-navy", valueClassName)}>
            {value}
          </strong>
        </div>
        {Icon ? (
...
export function StudentStatusBadge({ value }: { value: string }) {
  const variant = value === "Aprovado" ? "success" : value === "Cursando" ? "info" : value === "Reprovado" ? "danger" : "warning";
  return <Badge variant={variant}>{value}</Badge>;
}

export function StudentEmptyState({ title, description }: { title: string; description?: string }) {
  return (
